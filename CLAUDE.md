...
**줍스(JOOPS)** — 우주 쓰레기 청소 위성 펫 게임. 다마고치 스타일의 가상 펫 육성 + 방치형(Idle) + 액션 아케이드가 결합된 **하이브리드 캐주얼 모바일 웹 게임**입니다. 유저는 벤처 스타트업 '클리어 스카이(Clear Sky)'의 신입 오퍼레이터가 되어, **손안의 스마트폰을 관제 콘솔 삼아** 로우폴리(Low-Poly) 스타일의 인공위성 펫과 교감하며 우주 쓰레기를 청소하고 잃어버린 밤하늘의 별빛을 되찾습니다. (게임을 플레이하는 바로 그 기기가 세계관 속 관제 장비 — 모바일 디제시스가 정체성입니다.)
...
**현재 설치됨:** Next.js 15 (App Router), React 19, TypeScript (strict), Tailwind CSS v4, Framer Motion, Zustand, web-push, @supabase/supabase-js + @supabase/ssr, lottie-react. 경로 별칭 `@/*` → 저장소 루트. Tailwind v4는 `tailwind.config` 파일 없이 `app/globals.css`의 `@theme inline`으로 테마를 정의합니다.
...
**펫 비주얼 = 2D 스티커** (개발 일지 15편 참조): 홈 화면 줍이는 SVG 에셋 팩(`plan/img/satellite pet svg pack.zip`)의 카와이 스티커로 렌더합니다 — 캐릭터 5종 × 감정 프리베이크 6종이 `public/game/pet/`에 배치되어 있고, `components/home/pet-sticker.tsx`가 감정 유도(동면>절전>시무룩>데이터가득>배터리부족>평상)와 파일 스왑·Framer Motion 모션을 담당합니다. 제스처는 DOM 레이어(`pet-satellite.tsx`)가 받아 petting/burstNonce 신호로 전달. 한때 R3F+GLB 3D 펫이었으나 귀여움·발열 문제로 접었습니다(개발 일지 5·13편이 그 역사 — three 계열 의존성도 제거됨, GLB 원본은 zip에 보존). **푸시(배포) 시 `package.json` 버전을 올리세요** — PWA 업데이트 감지 트리거입니다.
...
## 협업 워크플로 (2026-07-21부터)

동료들과의 협업이 시작되어 브랜치 기반으로 전환했습니다:

1. **`main`은 공유 안정 브랜치 — 직접 커밋 금지.** 모든 작업은 작업 브랜치에서 하고 PR로 병합합니다.
2. **오너(이 세션)의 작업 브랜치는 `1`.** 동료들은 각자의 브랜치를 씁니다.
3. **병합은 GitHub PR로** (gh CLI 연동됨, 계정 ppabam). PR 제목·본문도 커밋처럼 한국어로 "무엇을, 왜"가 드러나게 작성합니다.
4. 작업 브랜치는 수시로 `main`을 리베이스/병합해 최신을 유지하고, 충돌은 작은 단위로 해소합니다.
5. 배포 버전 업 규칙(푸시 시 `package.json` 버전 올리기)은 `main` 병합 시점 기준으로 유지합니다.
